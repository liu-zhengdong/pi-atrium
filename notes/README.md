...
- 展开全文的单篇笔记正文超过 `maxNoteBytes`（默认 **8 KiB**，可设为 1 KiB 到 16 MiB）时仍整篇注入，但在正文前附一行给模型的提醒：按渐进式披露拆分，根笔记留要点和索引，细节移到带 `keywords` 的子目录分册。拆到上限以内后提醒自动消失。这条提醒只进模型上下文，不计入读取问题。
